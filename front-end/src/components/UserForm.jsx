...
const UserForm = ({
  ...
  const validate = () => {
    ...
    if (!email || !isValidEmail(email)) nextErrors.email = EMAIL_RULE_MESSAGE;
    if (!phone || !isValidPhone(phone)) nextErrors.phone = PHONE_RULE_MESSAGE;
    if (!formData.birthDate) nextErrors.birthDate = "Ngày sinh không được để trống.";

    if (!userData) {
      if (!formData.password) nextErrors.password = PASSWORD_RULE_MESSAGE;
